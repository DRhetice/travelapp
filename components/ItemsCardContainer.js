...
const ItemsCardContainer = ({ imageSrc, title, location, data }) => {
  ...
  return (
    <TouchableOpacity
     ...
     shadow-md bg-white w-[182px]  my-2 "
    >
      <Image
        source={{ uri: imageSrc }}
      ...
      {title ? (
        <>
          <Text className="text-[#4282BB] text-[18px] font-bold">
            {title?.length > 14 ? `${title.slice(0, 14)}..` : title}
          </Text>

          <View className="flex-row items-center space-x-2">
            <FontAwesome name="map-marker" size={20} color="#4282BB" />

            <Text className="text-[#4282BB] text-[14px] font-bold">
